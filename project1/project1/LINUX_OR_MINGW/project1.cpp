...
//        meshes[i] = NULL;
    ...
    shaderProgram = InitShader( "vStart2.glsl", "fStart2.glsl" );

    glUseProgram( shaderProgram ); CheckError();
	
   // GLint timeParam = glGetUniformLocation(shaderProgram, "time");
    ...
    sceneObjs[2].loc = vec4(2.0, 1.5, 1.0, 1.0);
...
void drawMesh(SceneObject sceneObj) {
    ...
    mat4 model = Translate(sceneObj.loc) * RotateX(sceneObj.angles[0]) * RotateY(sceneObj.angles[1]) * RotateZ(sceneObj.angles[2]) * Scale(sceneObj.scale) + (0.01* LSDMode * abs((sin(0.01 * timeParam + sceneObj.meshId)))) ;
...
// add appropriate rotations.
    ...
    view = Translate(0.0, -0.5, -viewDist);
	view = view * RotateY(camRotSidewaysDeg);
	view = view * RotateX(camRotUpAndOverDeg);
    ...
    SceneObject lightObj2 = sceneObjs[2]; // The actual light is just in front of the sphere.
    vec4 light2Position = view * ( lightObj2.loc - normalize(lightObj2.loc)*lightObj2.scale*1.05 );
    ...
    for(int i=0; i<nObjects; i++) {
		...
		so.scale = so.scale + LSDMode * sin(timeParam * 0.01);
        vec3 rgb = so.rgb * lightObj1.rgb * so.brightness * lightObj1.brightness * 2.0 * lightObj2.rgb * lightObj2.brightness;
        glUniform4fv( glGetUniformLocation(shaderProgram, "AmbientProduct"), 1, so.ambient * rgb ); CheckError();
        glUniform4fv( glGetUniformLocation(shaderProgram, "DiffuseProduct"), 1, so.diffuse * rgb );
        glUniform4fv( glGetUniformLocation(shaderProgram, "SpecularProduct"), 1, so.specular * vec3(1.0,1.0,1.0)
		);
	...
	glUniform1f(  glGetUniformLocation(shaderProgram, "time"), timeParam); CheckError();
	glClearColor( LSDMode * sin(0.01* timeParam), LSDMode * sin(0.01* timeParam), LSDMode * sin(0.01* timeParam), 1.0 ); 
    glutSwapBuffers();
...
static void materialMenu(int id) {
  ...
  if(id==20) setTool(&sceneObjs[currObject].ambient, &sceneObjs[currObject].diffuse, mat2(1, 0, 0, 1),
                     &sceneObjs[currObject].specular, &sceneObjs[currObject].shine, mat2(1, 0, 0, 100) );
...
static void makeMenu() {
  ...
  int customMenuId = glutCreateMenu(customMenu);
  glutAddMenuEntry("LSD                ", 100);
  ...
  glutAddSubMenu("Custom Functions", customMenuId);
