...
static vec2 clickOrigin;
static float *leftXYptr[2]={NULL,NULL};
static float *middXYptr[2]={NULL,NULL};
static vec2 leftXYold, middXYold;
static mat2 leftTrans, middTrans;
static int currButton = -1; 

static int mouseX=0, mouseY=0; // These need to be updated in scene.c


static float currRawX() { return ((float)mouseX)/glutGet(GLUT_WINDOW_WIDTH); }
static float currRawY() { 
    return ((float)(glutGet(GLUT_WINDOW_HEIGHT)-mouseY))/glutGet(GLUT_WINDOW_HEIGHT); 
}

static void doToolUpdateXY() { 
    if(currButton == GLUT_LEFT_BUTTON) {
        vec2 transformed = leftXYold + leftTrans * (vec2(currRawX(),currRawY()) - clickOrigin);
        if(leftXYptr[0]) { *(leftXYptr[0]) = transformed.x; }
        if(leftXYptr[1]) { *(leftXYptr[1]) = transformed.y; }
    } else if(currButton == GLUT_RIGHT_BUTTON) {
        vec2 transformed = middXYold + middTrans * (vec2(currRawX(),currRawY()) - clickOrigin);
        if(middXYptr[0]) { *(middXYptr[0]) = transformed.x; }
        if(middXYptr[1]) { *(middXYptr[1]) = transformed.y; }
    }
}
...
static void setTool(float* vX, float* vY, mat2 leftT, float *wX, float *wY, mat2 middT) {

    leftXYptr[0]=vX;   if(vX) { leftXYold[0] = *vX; }
    leftXYptr[1]=vY;   if(vY) { leftXYold[1] = *vY; }
    leftTrans = leftT; 
    middXYptr[0]=wX;   if(wX) { middXYold[0] = *wX; }
    middXYptr[1]=wY;   if(wY) { middXYold[1] = *wY; }
    middTrans = middT;
...
static void activateTool(int button) {
    currButton=button;
    clickOrigin = vec2(currRawX(), currRawY());
    if(leftXYptr[0]) { leftXYold[0] = *(leftXYptr[0]); }
    if(leftXYptr[1]) { leftXYold[1] = *(leftXYptr[1]); }
    if(middXYptr[0]) { middXYold[0] = *(middXYptr[0]); }
    if(middXYptr[1]) { middXYold[1] = *(middXYptr[1]); }
